...
The C++ extension holds on to ruby object references (VALUEs) -- for instance, the 'deflate'
procedures -- but ruby's garbage collector is unaware of these references. If they get collected,
all kinds of random nastiness ensues. Consequently, all VALUEs held in C++ are also added to
a ruby array held by the Pbr instance. This ensures they are not collected until the C++ code
